...
def cross_entropy(
    ...
    # top(Top of the fraction)
    # = log(exp(logits[expected]) - exp(logits.max)) = logits[expected] - logits.max. 
    # logits[expected]
    ...
    # Mean across all batches/sequences. We taking the negative log of top/bottom,
    # which equals taking the logs respectively and do a minus.
